...
{
    const char* resp_str = "<h1>Led Status: </h1><p>Visit /on to turn the LED on.</p><p>Visit /off to turn the LED off.</p>";
    ESP_LOGI(TAG, "Sending response: %s", resp_str);
    httpd_resp_send(req, resp_str, HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
